    ...
    <div class="sjb-container calendar-list-container">
        <div class="filter-container">
            ...
            <div class="toolbar-item">
                ...
                <el-select clearable style="width: 120px" class="filter-item" v-model="listQuery.resourcesHandleStatus" placeholder="请选择">
            ...
            <!-- <div class="toolbar-item">
                ...
                <el-input @keyup.enter.native="handleFilter" style="width: 180px;" class="filter-item" placeholder="请输入" v-model.trim="listQuery.procName">
                </el-input>
            </div> -->
            ...
            <div class="toolbar-item">
                ...
                <el-button class="filter-item" style="margin-left: 10px;" @click="handleCreate" type="primary" icon="el-icon-edit">新建资源申请办理</el-button>
            </div>
        ...
        <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row style="width: 100%">
            ...
            <el-table-column align="center" label="办理类型" width="80px">
                <template slot-scope="scope">
                    <span>{{scope.row.handleTypeValue}}</span>
                </template>
            </el-table-column>
            <el-table-column align="center" label="资源类型" width="80px">
                <template slot-scope="scope">
                    <span>{{scope.row.resourcesTypeValue}}</span>
                </template>
            </el-table-column>
            <el-table-column align="center" label="满足数量" width="100px">
                <template slot-scope="scope">
                    <span>{{scope.row.personelNum}}</span>
                </template>
            </el-table-column>
            <el-table-column width="150px" align="center" label="申请时间">
                <template slot-scope="scope">
                    <span class="ignore-detail">{{scope.row.applyTime | stamp2TextDateFull}}</span>
                </template>
            </el-table-column>
            <el-table-column width="80px" align="center" label="审批状态">
                <template slot-scope="scope">
                    <span>{{ scope.row.resourcesHandleStatusValue}}</span>
                </template>
            </el-table-column>
            <el-table-column align="center" label="操作" class-name="small-padding fixed-width" width="100px">
                <template slot-scope="scope">
...
import { 
    resourcesHandleFlowList,fetchProDic//Web端出差申请-查询列表
 } from "@/api/resHandle";
...
export default {
    ...
    data() {
        return {
            ...
            listQuery: {
                timeRange: [],
                // billType:4,//单据类型:1:市场,2:实施,3:报销,4:接待申请,5:出差申请
                
                procCode: "", // 流程编号 
                projectId: "", //项目名称
                resourcesHandleStatus: "", //申请状态   ~~~~~
                procName: "",  //流程名称
                start: "",
                end: "",
            }
    ...
    mounted() {
        ...
        this.expStatuList = selectDic(dicList, "expense_status"); //申请状态
    },
    methods: {
        getList() {
            var _this = this;
        ...
        reduceParams($$imData) {
            ...
            const $$postData = $$imData
                .set('start', common.rangeObjToTimestamp($$imData.get('timeRange').toJS()).applyTimeStart)
                .set('end', common.rangeObjToTimestamp($$imData.get('timeRange').toJS()).applyTimeEnd)
                .delete('timeRange')
        ...
        handleCreate() {
            this.$router.push({ path: "/me/resHandleForm" });
        }
    }
};
</script>
