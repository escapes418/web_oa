...
export default {
    ...
    async created() {
        if(this.$route.query.key){
            getDetail({
                ...
                if(res.data.remindType || res.data.remindType == 0){
                    this.postData.remindType = res.data.remindType
                }else{
                     this.postData.remindType = ""
                }
                this.postData.sendFrequency = res.data.sendFrequency;
                this.postData.remindWays = res.data.remindWays || [];
                this.postData.pollHourMin = res.data.pollHourMin || "";
                this.postData.pollWeek = res.data.pollWeek;
                this.postData.pollWeekday = res.data.pollWeekday || "";
                this.postData.pollMonth = res.data.pollMonth || "";
                this.postData.pollMonthday = res.data.pollMonthday || "";
                this.postData.emailBlack = res.data.emailBlack || [];
                this.postData.smsBlack= res.data.smsBlack || [];
                if(res.data.startTime){
                    this.startTime =  common.timeParseObj(res.data.startTime)
                }
                
                if(res.data.isWeek == 1){
                    this.isWeek = true
                }else{
                    this.isWeek = false;
                }
    ...
    methods: {
        ...
        goBack(){
            this.$router.push({path:'/remind/todoList' })
        },
        // 提交
        submit() {
            if(this.isWeek){
