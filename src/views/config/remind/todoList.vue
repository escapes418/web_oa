    ...
    <div class="sjb-container calendar-list-container">
        ...
        <el-table :data="list" border fit highlight-current-row style="width: 100%">
            ...
            <el-table-column align="center" label="频率" width="200px">
                <template slot-scope="scope">
                    <span>{{scope.row.sendFrequency =="0" ? "按天":scope.row.sendFrequency =="1" ? "按星期":scope.row.sendFrequency =="2"?"按月":scope.row.sendFrequency =="3"?"一次":""}}</span>
                </template>
            </el-table-column>
...
export default {
    ...
    methods: {
        ...
        handleCreate() {
            this.$router.push({ path: '/remind/todoForm' })
        },
        showDel(row){
