        ...
        <div class="segment-area">
          <div class="el-table__body-wrapper">
            <el-table :data="list" border>
              <el-table-column label="上传时间" prop="startDate">
                <template slot-scope="scope">
                  <span>{{ scope.row.createTime | stamp2TextDateFull }}</span>
                </template>
              </el-table-column>
              <el-table-column label="附件名称" prop="endDate">
                <template slot-scope="scope">
                  <span>{{ scope.row.fileName }}</span>
                </template>
              </el-table-column>
              <el-table-column label="上传人" prop="expenseAmt">
                <template slot-scope="scope">
                  <span class="font-orange">{{ scope.row.createName }}</span>
                </template>
              </el-table-column>
              <el-table-column label="上传操作" prop="endDate">
                <template slot-scope="scope">
                  <span>{{ scope.row.operateReason }}</span>
                </template>
              </el-table-column>
              <el-table-column label="操作" prop="endDate">
                <template slot-scope="scope">
        ...
        <div class="segment-area">
          <div class="el-table__body-wrapper">
            <el-table :data="sonList" border>
              <el-table-column label="任务编号" prop="startDate">
                <template slot-scope="scope">
                  <span>{{ scope.row.taskCode }}</span>
                </template>
              </el-table-column>
              <el-table-column label="任务名称" prop="endDate">
                <template slot-scope="scope">
                  <el-button type="text" @click="goSon(scope.row)">{{ scope.row.taskName }}</el-button>
                </template>
              </el-table-column>
              <el-table-column label="所属阶段" prop="expenseAmt">
                <template slot-scope="scope">
                  <span class="font-orange">{{ scope.row.stageName }}</span>
                </template>
              </el-table-column>
              <el-table-column label="状态" prop="endDate">
                <template slot-scope="scope">
                  <span>{{ scope.row.taskStatus == 0 ? "进行中" : "已完成" }}</span>
                </template>
              </el-table-column>
              <el-table-column label="任务责任人" prop="endDate">
                <template slot-scope="scope">
                  <span>{{ scope.row.principalName }}</span>
                </template>
              </el-table-column>
              <el-table-column label="进度" prop="endDate">
                <template slot-scope="scope">
              ...
              <el-table-column label="完成日期" prop="endDate">
                <template slot-scope="scope">
            ...
            <div class="pagination-container">
              <el-pagination
                ...
                :current-page="sonpageNum"
                :page-size="sonPageSize"
                layout="total, prev, pager, next, jumper"
                :total="sonTotal"
              >
              </el-pagination>
  ...
  methods: {
    ...
    showImg(index, type) {
      if (type == 1) {
        ...
        // 如果已初始化过 直接show出
        if (this.list[index].viewer) {
          this.list[index].viewer.show();
          return;
          // this.clientimg[index].viewer.destroy();//也可以销毁后再重新初始化
        }
        // 初始化
        this.list[index].viewer = new Viewer(event.target, {});
        // 调用show
        this.list[index].viewer.show();
      } else {
        if (this.list[index].viewer) {
